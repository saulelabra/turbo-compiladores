...
getArrAt(){
    ...
    double *element_ptr = d1.sym->u.arr;
    double element = *(element_ptr + (int)d2.val);
    printf("stored value: %s %f\td2: %f\n", d1.sym->name, element, d2.val);
...
assignInArr() {
    ...
    double *arr_element_ptr = d1.sym->u.arr + (int)d3.val;

    *arr_element_ptr = d2.val;
    d1.sym->type = VAR;
    push(d1);

    printf("assigned value: %f\n", d1.sym->u.arr[(int)d3.val]);
}
